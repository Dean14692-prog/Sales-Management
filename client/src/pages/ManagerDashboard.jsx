    ...
    <div className="h-screen overflow-hidden flex">
      ...
      <div className="flex-1 p-6 overflow-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          {/* 7. Genset Model Performance */}
          <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                Genset Model Performance
              </h3>
              <BarChart3
                className="h-6 w-6"
                style={{ color: onanGreen.primary }}
                ...
                <BarChart data={gensetModelData}>
                  ...
                  <Tooltip
                    formatter={(value, name) => [
                      ...
                      name === "units" ? "Units Sold" : "Total Revenue",
                    ]}
                  />
                  <Bar dataKey="units" fill={onanGreen.light} />
                  <Bar dataKey="total" fill={onanGreen.primary} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>

          {/* Quarterly Target (now uses actualQuarterlyTarget) */}
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">
                Q{currentQuarter} Sales Target ({selectedYear}){" "}
                {/* Dynamic title */}
              </h3>
              <Target
                className="h-6 w-6"
            ...
            <div className="space-y-3">
              <div className="w-full bg-gray-200 rounded-full h-3">
                <div
                  ...
                  style={{
                    width: `${quarterlyPercentage}%`,
                    backgroundColor: onanGreen.primary,
                  }}
                ></div>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">
                  {formatCurrency(currentQuarterlySales)} /{" "}
                  {formatCurrency(actualQuarterlyTarget)}
                </span>
                <span className="text-sm font-semibold text-gray-900">
                  {quarterlyPercentage.toFixed(0)}%
                </span>
              </div>
              {/* This message now dynamically shows the percentage needed */}
              {currentQuarterlySales < actualQuarterlyTarget && (
                <p className="text-xs text-orange-600 font-medium">
                  Needs {percentageNeeded.toFixed(0)}% to reach target
                </p>
              )}
              {currentQuarterlySales >= actualQuarterlyTarget && (
